...
export default function Home() {
  ...
  // Use a fallback if environment variable is not defined, for static export
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const totalFileSize =
      publicKeyFile.size + hashFile.size + signatureFile.size;
    ...
    try {
      ...
      xhr.open("POST", `${backendUrl}/verify`);
      xhr.send(formData);
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      {/* Fixed top-right header with the GitHub link */}
      <header className="absolute top-0 right-0 p-4">
        <Link href="https://github.com/ShashiSugrim" legacyBehavior>
      ...
      {/* Add top padding so main content is pushed down */}
      <main className="pt-8 max-w-2xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={
              isLoading || !publicKeyFile || !hashFile || !signatureFile
            }
            className={`
          ...
          {isLoading && (
            <div className="w-full mt-4">
              ...
              <div className="text-sm text-gray-600 text-center">
                ...
                {totalSize > 0 && (
                  <span className="ml-2">
                    ({(loadedSize / 1024 / 1024).toFixed(2)}MB /{" "}
